...
FOR %workfile {%4} {%5} {%6} {%7} {%8} {%9}
        IF @len(%workfile) > 0 THEN 
                wfselect {%workfile}
		' make sure workfile contains forecast dates
		pagestruct(end=%endforecastyear, regular)
		' set up table to hold results
				...
				series diffs = @exp(tempseries2) - @exp({%depvar})
				series squarediffs = diffs^2
				resultstable(!resultsrow,6) = @mean(squarediffs)
				resultstable(!resultsrow,7) = @mean(@abs(diffs))
				' forecast 
				smpl {%forecastsample}
				{%equationname}.forecast tempseries2 tempseries1
				...
				smpl {%testsample}
				{%equationname}.forecast tempseries2 tempseries1
				series diffs = tempseries2 - {%depvar}
				series squarediffs = diffs^2
				resultstable(!resultsrow,6) = @mean(squarediffs)
				resultstable(!resultsrow,7) = @mean(@abs(diffs))
				' forecast 
				smpl {%forecastsample}
				{%equationname}.forecast tempseries2 tempseries1
				series {%predictionname} = tempseries2
				series {%upperboundname} = tempseries2 + 2*tempseries1
				series {%lowerboundname} = tempseries2 - 2*tempseries1
